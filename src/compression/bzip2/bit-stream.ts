...
const BitStream = function(stream) {
    (function() {
        let bufferByte = 0x100; // private var for readers
        this.readBit = function() {
            if ((bufferByte & 0xFF) === 0) {
                let ch = stream.readByte();
                if (ch === Stream.EOF) {
                    this._eof = true;
                    return ch; /* !!! */
                }
        ...
        this.seekBit = function(pos) {
            let n_byte = pos >>> 3;
            let n_bit = pos - (n_byte*8);
            this.seek(n_byte);
            this._eof = false;
            this.readBits(n_bit);
        };
        this.tellBit = function() {
            let pos = stream.tell() * 8;
            let b = bufferByte;
            while ((b & 0xFF) !== 0) {
                pos--;
                b <<= 1;
    ...
    (function() {
        let bufferByte = 1; // private var for writers
...
BitStream.prototype.writeBits = function(n, value) {
    if (n > 32) {
        ...
        let high = (value - low) / (0x10000); // fp division, not shift
