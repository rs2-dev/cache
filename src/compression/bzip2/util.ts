...
const BufferStream = function(buffer, resizeOk) {
    this.buffer = buffer;
...
BufferStream.prototype.writeByte = function(_byte) {
    if (this.resizeOk && this.pos >= this.buffer.length) {
        let newBuffer = Util.makeU8Buffer(this.buffer.length * 2);
...
BufferStream.prototype.getBuffer = function() {
    ...
    if (this.pos !== this.buffer.length) {
        ...
        let newBuffer = Util.makeU8Buffer(this.pos);
...
Util.coerceOutputStream = function(output, size) {
    let r = { stream: output, retval: output };
    if (output) {
...
Util.compressFileHelper = function(magic, guts, suppressFinalByte) {
    return function(inStream, outStream, props) {
        ...
        if (suppressFinalByte) {
            let tmpOutput = Util.coerceOutputStream([]);
...
Util.decompressFileHelper = function(magic, guts) {
    return function(inStream, outStream) {
        ...
        let fileSize = Util.readUnsignedNumber(inStream) - 1;
        let o = Util.coerceOutputStream(outStream, fileSize);
        outStream = o.stream;
...
const fallbackarray = function(size) {
...
let ensureZeroed = function id(a) { return a; };
