...
const getCounts = function(T, C, n, k) {
    let i;
    for (i = 0; i < k; i++) { C[i] = 0; }
    for (i = 0; i < n; i++) { C[T[i]]++; }
};
const getBuckets = function(C, B, k, end) {
    let i, sum = 0;
    if (end) {
...
const LMSsort = function(T, SA, C, B, n, k) {
    let b, i, j;
    let c0, c1;
    /* compute SAl */
...
const LMSpostproc = function(T, SA, n, m) {
    let i, j, p, q, plen, qlen, name;
    let c0, c1;
    let diff;
...
/* compute SA and BWT */
const induceSA = function(T, SA, C, B, n, k) {
    let b, i, j;
    let c0, c1;
    /* compute SAl */
    if (C === B) { getCounts(T, C, n, k); }
...
const computeBWT = function(T, SA, C, B, n, k) {
    let b, i, j, pidx = -1;
    let c0, c1;
    /* compute SAl */
    if (C === B) { getCounts(T, C, n, k); }
...
/* find the suffix array SA of T[0..n-1] in {0..k-1}^n
   ...
   constant alphabet */
const SA_IS = function(T, SA, fs, n, k, isbwt) {
    let C, B, RA;
    let i, j, b, c, m, p, q, name, pidx = 0, newfs;
    let c0, c1;
    let flags = 0;
