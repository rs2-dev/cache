...
import BWT from './bwt';
import allocateHuffmanCodeLengths from './huffman-allocator';
...
const WHOLEPI = 0x314159265359; // 48-bit integer
const SQRTPI =  0x177245385090; // 48-bit integer

const EOF = Stream.EOF;

const mtf = function(array, index) {
    let src = array[index], i;
    for (i = index; i > 0; i--) {
        array[i] = array[i-1];
    }
    array[0] = src;
...
const _throw = function(status, optDetail?) {
    ...
    let e = new TypeError(msg);
    (e as any).errorCode = status;
    throw e;
};
...
Bunzip.prototype._init_block = function() {
    let moreBlocks = this._get_next_block();
    if ( !moreBlocks ) {
        this.writeCount = -1;
...
Bunzip.prototype._start_bunzip = function(inputStream, outputStream) {
    ...
    let buf = Util.makeU8Buffer(4);
    if (inputStream.read(buf, 0, 4) !== 4 ||
        String.fromCharCode(buf[0], buf[1], buf[2]) !== 'BZh')
        _throw(Err.NOT_BZIP_DATA, 'bad magic');

    let level = buf[3] - 0x30;
    if (level < 1 || level > 9)
...
Bunzip.prototype._get_next_block = function() {
    ...
    let origPointer = reader.readBits(24);
    ...
    let t = reader.readBits(16);
    let symToByte = Util.makeU8Buffer(256), symTotal = 0;
    for (i = 0; i < 16; i++) {
        if (t & (1 << (0xF - i))) {
            let o = i * 16;
    ...
    let groupCount = reader.readBits(3);
    ...
    /* nSelectors: Every GROUP_SIZE many symbols we select a new Huffman coding
       ...
       start of the list.) */
    let nSelectors = reader.readBits(15);
    if (nSelectors === 0)
        _throw(Err.DATA_ERROR);

    let mtfSymbol = Util.makeU8Buffer(256);
    for (i = 0; i < groupCount; i++)
        mtfSymbol[i] = i;

    let selectors = Util.makeU8Buffer(nSelectors); // was 32768...
    ...
    for (j = 0; j < groupCount; j++) {
        let length = Util.makeU8Buffer(symCount), temp = Util.makeU16Buffer(MAX_HUFCODE_BITS + 1);
    ...
    let byteCount = Util.makeU32Buffer(256);
    for (i = 0; i < 256; i++)
        mtfSymbol[i] = i;
    /* Loop through compressed symbols. */
    let runPos = 0, dbufCount = 0, selector = 0, uc;
    let dbuf = this.dbuf = Util.makeU32Buffer(this.dbufSize);
    symCount = 0;
    for (;;) {
...
Bunzip.decode = function(input, output, multistream) {
    ...
    let inputStream = Util.coerceInputStream(input);
    let o = Util.coerceOutputStream(output, output);
    let outputStream = o.stream;
    ...
    while (true) {
        ...
        if (bz._init_block()) {
            ...
            let targetStreamCRC = bz.reader.readBits(32);
...
Bunzip.decodeBlock = function(input, pos, output) {
    ...
    let moreBlocks = bz._get_next_block();
    if (moreBlocks) {
...
Bunzip.table = function(input, callback, multistream) {
    ...
    let bz = new Bunzip(inputStream, outputStream);
    let blockSize = bz.dbufSize;
    while (true) {
        if ('eof' in inputStream && inputStream.eof()) break;

        let position = bz.reader.tellBit();

        if (bz._init_block()) {
            let start = outputStream.pos;
            bz._read_bunzip();
            callback(position, outputStream.pos - start);
        } else {
            const crc = bz.reader.readBits(32); // (but we ignore the crc)
            if (multistream &&
                'eof' in inputStream &&
                !inputStream.eof()) {
                // note that start_bunzip will also resync the bit reader to next byte
...
const StaticHuffman = function(freq, alphabetSize) {
...
StaticHuffman.prototype.computeCanonical = function() {
    let alphabetSize = this.codeLengths.length;
    ...
    for (i=0; i<alphabetSize; i++) {
        let curLen = merged[i] >>> 9;
        let sym = merged[i] & 0x1FF;
...
StaticHuffman.prototype.emit = function(outStream) {
    ...
    let i, currentLength = this.codeLengths[0];
    outStream.writeBits(5, currentLength);
    for (i=0; i<this.codeLengths.length; i++) {
        let codeLength = this.codeLengths[i];
        let value, delta;
...
const readBlock = function(inStream, block, length, crc) {
    let pos = 0;
    let lastChar = -1;
    let runLength = 0;
    while (pos < length) {
...
const assignSelectors = function(selectors, groups, input) {
    let i, j, k;
    for (i=0, k=0; i<input.length; i+=GROUP_SIZE) {
        let groupSize = Math.min(GROUP_SIZE, input.length - i);
        let best = 0, bestCost = groups[0].cost(input, i, groupSize);
        for (j=1; j<groups.length; j++) {
            let groupCost = groups[j].cost(input, i, groupSize);
...
const optimizeHuffmanGroups = function(groups, targetGroups, input,
    ...
    while (groups.length < targetGroups) {
        ...
        let which = groupCounts.indexOf(Math.max.apply(Math, groupCounts));
        // ok, let's look at the size of those blocks
        let splits = [];
        for (i=0, j=0; i<selectors.length; i++) {
            if (selectors[i] !== which) { continue; }
            let start = i*GROUP_SIZE;
            let end = Math.min(start + GROUP_SIZE, input.length);
...
const compressBlock = function(block, length, outStream) {
    let c, i, j, k;
    // do BWT transform
    let U = Util.makeU8Buffer(length);
    let pidx = BWT.bwtransform2(block, U, length, 256);
    ...
    let A = Util.makeU16Buffer(length+1);
    let endOfBlock = alphabetSize + 1;
    let freq = [];
    for (i=0; i<=endOfBlock; i++) { freq[i] = 0; }
    let M = Util.makeU8Buffer(alphabetSize);
    for (i=0, j=0; i<256; i++) {
        if (used[i]) { M[j++] = i; }
    }
    used = null; compact = null;
    let pos = 0, runLength = 0;
    let emit = function(c) {
        A[pos++] = c;
        freq[c]++;
    };
    let emitLastRun = function() {
        while (runLength !== 0) {
            if (runLength & 1) {
                emit(0); // RUNA
                runLength -= 1;
    ...
    let groups = [];
    let targetGroups; // how many Huffman groups should we create?
    ...
    let selectors = Util.makeU8Buffer(Math.ceil(pos / GROUP_SIZE));
...
const Bzip2 = Object.create(null);
Bzip2.compressFile = function(inStream, outStream, props) {
    inStream = Util.coerceInputStream(inStream);
    let o = Util.coerceOutputStream(outStream, outStream);
    outStream = new BitStream(o.stream);

    let blockSizeMultiplier = 9;
    ...
    let blockSize = blockSizeMultiplier * 100000;
    ...
    let block = Util.makeU8Buffer(blockSize);
    let streamCRC = 0;
    let length;
